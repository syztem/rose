 ...
 * Quality: denser 48×24 dual-shell loft, 40 petals, lobe/ruffle silhouette ≠ runtime procedural.
 * Limits: parametric loft only (not hand-authored Blender sculpt); 512 bake maps; undraco GLB.
 */
...
function makePetalGeometry(opts = {}) {
  ...
  const SEG_U = 24;
  const LEN = 0.82;
  const T_HALF_MIN = 0.0018;
  const T_HALF_MAX = 0.0075;
  ...
  function centerPos(u, v, out) {
    const w = halfWidth(u);
    let x = (v - 0.5) * 2 * w;
    // Lobe asymmetry + tip lean (v2 silhouette ≠ procedural)
    x += tipLean * u * u * u;
    x += Math.sin((v - 0.5) * Math.PI * 2.2) * 0.018 * u * u * bellyBias;
    const serr = 0.006 * Math.sin(v * Math.PI * 11) * Math.pow(Math.abs(v - 0.5) * 2, 1.6) * u;
    x += serr;
    const y = u * LEN;
    const edge = Math.abs(v - 0.5) * 2;
    const mid = Math.exp(-(edge * 3.2) * (edge * 3.2));
    const cup = u * u * 0.16 * cupBias;
    const edgeRoll = edge * edge * u * 0.13;
    const vein = mid * 0.022 * Math.sin(u * Math.PI) * veinBias;
    const veinRipple = 0.007 * Math.sin(u * 16) * mid * veinBias;
    const lateral = 0.01 * Math.sin(u * 7 + v * 9) * edge * veinBias;
    const tipCurl = u * u * u * 0.055 * tipCurlBias;
    const ruffle = 0.012 * Math.sin(u * 22 + seed) * Math.pow(u, 1.4) * edge;
    const z = cup + edgeRoll - vein + veinRipple + lateral + tipCurl + ruffle;
...
function buildRose() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const t = i / (count - 1);
    const yaw = i * GOLDEN;
    const radius = 0.01 + 0.168 * Math.pow(t, 0.62);
    const open = 1 / (1 + Math.exp(-(t - 0.28) * 10.5));
    const pitch = THREE.MathUtils.lerp(0.18, 1.32, open);
    const lift = 0.12 + t * 0.58 + (1 - open) * 0.12;

    mesh.position.set(Math.cos(yaw) * radius, lift, Math.sin(yaw) * radius);
    mesh.rotation.order = 'YXZ';
    mesh.rotation.y = yaw;
    mesh.rotation.x = pitch;
    mesh.rotation.z = (t - 0.5) * 0.18 + Math.sin(i * 1.7) * 0.04;
    mesh.userData.baseRotZ = mesh.rotation.z;
    const s = THREE.MathUtils.lerp(0.42, 1.08, open);
    mesh.scale.setScalar(s * THREE.MathUtils.lerp(0.88, 1.22, Math.sin(t * Math.PI)));
...
function bakeMaps(size = 512) {
  ...
  for (let y = 0; y < size; y++) {
    ...
    for (let x = 0; x < size; x++) {
      ...
      // Stretch contrast so thicknessMap actually modulates attenuation (smooth gradients compress tiny).
      let tNorm = THREE.MathUtils.clamp((fullT - T_MIN) / (T_MAX - T_MIN), 0, 1);
      tNorm = Math.pow(tNorm, 0.72);
      tNorm = THREE.MathUtils.clamp(tNorm * 1.15 - 0.08, 0, 1);

      // roughness: base glass ~0.08; edges/tip wear higher
      const rough =
        0.05 +
        0.34 * edgeThin * (0.5 + 0.5 * tip) +
        0.12 * tip +
        0.07 * (1 - mid) * Math.sin(u * 18) * Math.sin(u * 18);

      // veins: midrib + lateral secondary (grayscale height for bump)
      const primary = mid * (0.65 + 0.35 * Math.sin(u * Math.PI));
      const lateral =
        0.55 *
        Math.exp(-Math.pow((edge - 0.32) * 4.5, 2)) *
        Math.abs(Math.sin(u * 11 + edge * 2.4));
      const tipVeins = tip * 0.35 * Math.abs(Math.sin(v * Math.PI * 7));
      const tertiary = 0.25 * mid * Math.abs(Math.sin(u * 21 + v * 13));
      const vein = THREE.MathUtils.clamp(primary * 0.85 + lateral * 0.7 + tipVeins + tertiary, 0, 1);
...
async function main() {
  ...
  const manifest = {
    ...
    quality:
      'v2 parametric 48×24 dual-shell loft, 40 petals, lobe/ruffle silhouette (not Blender sculpt)',
    limits: [
  ...
  // Ship into public/ so the static app picks them up with no flags
  const pub = path.resolve(__dirname, '../public');
  fs.mkdirSync(path.join(pub, 'models'), { recursive: true });
  fs.mkdirSync(path.join(pub, 'textures'), { recursive: true });
  fs.copyFileSync(glbPath, path.join(pub, 'models', 'rose.glb'));
  fs.copyFileSync(path.join(OUT, 'textures', 'rose_thickness.png'), path.join(pub, 'textures', 'rose_thickness.png'));
  fs.copyFileSync(path.join(OUT, 'textures', 'rose_roughness.png'), path.join(pub, 'textures', 'rose_roughness.png'));
  fs.copyFileSync(path.join(OUT, 'textures', 'rose_veins.png'), path.join(pub, 'textures', 'rose_veins.png'));
  fs.writeFileSync(path.join(pub, 'models', 'rose.manifest.json'), JSON.stringify(manifest, null, 2));

  console.log(JSON.stringify(manifest, null, 2));
  console.log('wrote', glbPath);
  console.log('shipped public/models/rose.glb + bake maps');
}
